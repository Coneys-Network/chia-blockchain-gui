...
function getInstallerConfig () {
  ...
  return Promise.resolve({
    ...
    noMsi: true,
    iconUrl: 'https://raw.githubusercontent.com/Coneys-Network/chia-blockchain-gui/dev/src/assets/img/csr.ico',
    outputDirectory: path.join(outPath, 'windows-installer'),
    certificateFile: 'win_code_sign_cert.p12',
    certificatePassword: process.env.WIN_CODE_SIGN_PASS,
    exe: 'Chia.exe',
    setupExe: 'ChiaSetup-' + process.env.CHIA_INSTALLER_VERSION + '.exe',
    setupIcon: path.join(rootPath, 'src', 'assets', 'img', 'csr.ico')
  })
}
